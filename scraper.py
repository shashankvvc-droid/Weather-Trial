...
class AccuWeatherClient:
    ...
    def get_daily_forecast(self, display_name: str, location_key: str) -> Optional[Dict]:
        print(f"Fetching 5-day forecast for: {display_name} (Key: {location_key})")
    ...
    def get_hourly_forecast(self, display_name: str, location_key: str) -> Optional[List]:
        print(f"Fetching 12-hour forecast for: {display_name} (Key: {location_key})")
    ...
    def fetch_city_data(self, city_config: Dict) -> Optional[Dict]:
        """Fetches both daily and hourly data for a single location."""
        try:
            # 🎯 FIXED: Cleanly reading from the new CSV columns
            city = city_config.get('City', 'Unknown City')
            zone = city_config.get('zone', 'Unknown Zone')
            loc_key = city_config.get('location_key')

            if not loc_key:
                print(f"WARNING: Location key not found in CSV for {zone} {city}. Skipping.")
                return None

            city_details = {'city': city, 'zone': zone}
            
            # Reconstruct the display name for the API logs
            ...
            print(f"ERROR: Error processing city {city_config.get('City', 'Unknown')}: {e}")
            return None
    ...
    def __init__(self, config: Config):
        try:
            ...
            if not creds_json:
                ...
                # 🎯 FIXED: Keep the newline safety net for GitHub Actions
...
def run_weather_processing(config: Config):
    ...
    for city in cities:
        try:
            ...
            # 🎯 FIXED: Safe error log retrieval matching new CSV structure
            print(f"CRITICAL ERROR processing {city.get('City', 'Unknown City')} ({city.get('zone', 'Unknown Zone')}): {e}")
        time.sleep(1)
